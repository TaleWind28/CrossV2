...
public class UpdateCredentials implements Values{
    ...
    public ServerMessage execute(JsonAccessedData data,String user,GenericTask task)throws ClassCastException{
        ...
        if(retvalue == 400)return new ServerMessage("Password non valida",102);
        ...
        if(userbook.getUserMap().get(this.username).getLogged()==true)return new ServerMessage("Non puoi cambiare la password di un utente loggato",105);
        //aggiorno l'userbook
